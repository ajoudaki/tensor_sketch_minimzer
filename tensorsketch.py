...
class MH:
    ...
    def sketch_all(self, seqs):
        return [self.sketch(s) for s in seqs]
...
@jitclass([
    ...
    ('signs', nb.float32[:, :])])
class TSM:
    def __init__(self, t, D, A, S, W, normalize=True, sample=ord('f')):
        self.A = A
        self.t = t
        self.D = D
        self.S = S
    ...
    def _full_sketch(self, seq: nb.int32[:]):
        ...
        T = np.zeros((self.t + 2, self.t + 2, self.D), dtype=np.float64)
        _sketch = np.zeros(shape=(len(seq), self.D))
        for p in range(1, self.t + 2):
            T[p][p - 1][0] = 1

        for i, c in enumerate(seq):
            for p in range(1, self.t + 1):  # p <- 1,..,t
                for q in range(self.t, p - 1, -1):  # q <- t,t-1,..,p
                    h = self.hashes[c][q - 1]
                    s = self.signs[c][q - 1]
                    for l in range(self.D):
                        r = l + h if l + h < self.D else l + h - self.D
                        T[p][q][r] += s * T[p][q - 1][l]
            # if i > window remove character seq[i-W]
            if i >= self.W:
                rc = seq[i - self.W]
                # iterate in the reverse order
                for p in range(self.t, 0, -1):  # p <- t, ..., 1
                    for q in range(p, self.t + 1, +1):  # q <- p,p+1,..,t
                        h = self.hashes[rc][p - 1]
                        s = self.signs[rc][p - 1]
                        for l in range(self.D):
                            r = l + h if l + h < self.D else l + h - self.D
                            T[p][q][r] -= s * T[p + 1][q][l]

            _sketch[i, :] = T[1][self.t][:]
        self._normalize(_sketch)
        return _sketch

    def sketch(self, seq: nb.int32[:]):
        _sk = self._full_sketch(seq)
        _min_indices = np.arange(1)
        if self.sample == ord('f'):
            _min_indices = np.arange(len(seq))
        elif self.sample == ord('s'):
            _min_indices = np.arange(self.S - 1, len(seq), self.S)
        elif self.sample == ord('m'):
            norms = np.sum(_sk ** 2, axis=1)
            _min_indices = minimizers(norms, self.S)
        return _sk[_min_indices, :], _min_indices

    def _normalize(self, sketch):
        if self.normalize:
            for i in range(sketch.shape[0]):
                sketch[i] = sketch[i] / np.linalg.norm(sketch[i], ord=1)

    def sketch_all(self, seqs):
        return [self.sketch(seq) for seq in seqs]
...
class TS:
    ...
    def sketch(self, seq: np.ndarray) -> nb.float32[:]:
...
class TE:
    ...
    def sketch(self, seq: nb.int32[:]) -> nb.float32[:]:
        full_sketch = self._full_sketch(seq)
        if self.normalize:
            # Normalization factor.
            n = len(seq)
    ...
    def sketch_all(self, seqs: list[nb.int32[:]]) -> list[nb.float32[:]]:
        return [self.sketch(seq) for seq in seqs]
...
def _gpu_sketch(A, t, D, L, hashes, signs, seq, starts, tuple_len):
    seq_id = cuda.blockIdx.x
    start = starts[seq_id]
    end = starts[seq_id + 1]
    ...
    # Copy to result.
    for ll in range(l, D, D // L):
        tuple_len[seq_id][k][ll] = Tin[read_plane + ll]
        tuple_len[seq_id][k + 1][ll] = Tin[read_plane + D + ll]
...
class cuTS:
    ...
    def sketch_all(self, seqs: list[nb.int8[:]]) -> list[nb.float32[:]]:
    ...
    def sketch(self, seq: nb.int8[:]) -> nb.float32[:]:
        return self.sketch_all(List([seq]))[0]
...
class TSS:
    def __init__(self, t, W, S, D, A, normalize=True, sketch_class=TS):
        # reduce sketch dim to ensure that the flattened sketch size is D
        D2 = D  # int(D/int((seq_len-W)/S)+1)
    ...
    def sketch(self, seq: nb.int32[:], flatten=True) -> nb.float32[:, :]:
        ...
        for si, i in enumerate(np.arange(0, len(seq) - self.W + 1, self.S)):
            sketch[:, si] = self.sketcher.sketch(seq[i:i + self.W])
        if flatten:
    ...
    def sketch_all(self, seqs):
        ...
        all_sketches = self.sketcher.sketch_all(all_seqs)
        sketches = List()
        for i in range(n):
...
# last two columns mutation rate is 1  -> totally random
def gen_seqs(N, A, num, num_rates):
    r = np.linspace(1.0 / num_rates, 1, num_rates)
    r[-2:] = r[-1]
    s = np.random.randint(0, A, size=(num, len(r) + 1, N), dtype=np.int8)
    for i in tqdm.tqdm(range(num), total=num, desc='generating seqs '):
        for j, mr in enumerate(r):
            s[i, j + 1, :] = mutate_seq(A=A, seq=s[i, 0, :], mr=mr)
    return s, r
...
class ProgressBarParallel:
    ...
    def inc(self):
        ...
        if (self.i % self.step == 0) or (self.i == self.n):
            ...
            percent = int(self.i * 100 / self.n)
            s = '\r' + self.desc + ': ' + str(percent) + '% '
...
def calc_dists(models, all_samples):
    ...
    for name, sketcher in trange:
        ...
        all_sketches = sketcher.sketch_all(all_seqs)
...
def plot_dists(rates, results, plot_log):
    plt.figure()
    for name, dists in results.items():
        ...
        detect[detect == 0] = 1.0 / dists.shape[0]  # 0 statistically means <= 1/#samples
        if plot_log:
